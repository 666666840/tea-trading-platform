...
Page({
  ...
  onPickerChange(e) {
    ...
    switch (field) {
      ...
      case 'harvestTime':
        ...
        break
      case 'budgetUnit':
        selectedValue = this.data.budgetUnits[value]
        break
      case 'quantityUnit':
        selectedValue = this.data.quantityUnits[value]
        break
      case 'urgency':
        selectedValue = this.data.urgencyOptions[value]
        break
      case 'delivery':
  ...
  // 验证所有字段
  validateAll() {
    const fields = ['title', 'teaTypes', 'origin', 'grade', 'budget', 'quantity', 'contact', 'phone']
    let allValid = true
